...
const handleSendMessage = (e)=>{
  ...
  return (
    <div style={{ backgroundImage: `url(${backgroundImage})` }} className="bg-no-repeat bg-cover">
      <header className="sticky top-0 h-16 bg-gray-800 text-white flex justify-between items-center px-4">
      ...
      <section className="h-[calc(100vh-128px)] overflow-x-hidden overflow-y-scroll scrollbar relative bg-gray-900 bg-opacity-50">
        <div className="flex flex-col gap-2 py-2 mx-2" ref={currentMessage}>
          {allMessage.map((msg, index) => (
            <div key={index} className={`p-1 py-1 rounded w-fit max-w-[280px] md:max-w-sm lg:max-w-md ${user._id === msg?.msgByUserId ? "ml-auto bg-teal-600 text-white" : "bg-white"}`}>
        ...
        {message.imageUrl && (
          <div className="w-full h-full sticky bottom-0 bg-gray-700 bg-opacity-40 flex justify-center items-center rounded overflow-hidden">
      ...
      <section className="h-16 bg-gray-800 flex items-center px-4">
        ...
        <form className="h-full w-full flex gap-2" onSubmit={handleSendMessage}>
          <input
            ...
            className="py-1 px-4 outline-none w-full h-full bg-gray-800 text-white"
            value={message.text}
